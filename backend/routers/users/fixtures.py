...
async def override_game_dep(game_id: int) -> Game:
    assert game_id == 1
...
async def override_team_dep(team_secret: str = Query(description="Team secret", default=None)):
    assert team_secret == "secret"
    return Team(team_id=1, team_name="team_1", game_id=1, team_secret="secret")


async def mock_check_game_active_dep(game: Game = Depends(game_dep)):
    mock_check_game_active_dep.call_count += 1
    return await check_game_active_dep(game)


async def mock_player_dep(game: Game = Depends(game_dep)):
...
async def mock_start_end_tick_dep(game: Game = Depends(game_dep),
    ...
    return await start_end_tick_dep(game, start_tick, end_tick)
