...
from .dependencies import check_game_active_dep, player_dep, game_dep
...
async def buy_plant(
    plant: PowerPlantTypeData, player: Player = Depends(player_dep), game=Depends(game_dep)
) -> SuccessfulResponse:
    """
...
async def turn_on(
    plant: PowerOn, player: Player = Depends(player_dep), game=Depends(game_dep)
) -> SuccessfulResponse:
    async with game_id_lock.lock(game.game_id):
        player_id = player.player_id
        player = await Player.get(player_id=player_id)
