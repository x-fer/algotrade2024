...
class Bots:
    bots = {"resource_bot": ResourceBot}

    def list():
        return [{"name": bot_name} for bot_name, bot_id in Bots.bots.items()]
    ...
    def parse_string(bot_string: str) -> List[Tuple[str, int]]:
        ...
        for bot, ammount in bots:
            if not Bots.exists(bot):
                raise HTTPException(400, "Bot does not exist")
            if ammount < 0 or ammount > 10:
                raise HTTPException(400, "Bot ammount is not in range [0, 10]")
