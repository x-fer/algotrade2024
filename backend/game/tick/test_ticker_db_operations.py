...
async def test_get_tick_data(
    ...
    with patch("model.Player.find", new=mock_list_players), patch(
        "model.Order.find", new=mock_list_orders
    ), patch("model.DatasetData.get", new=mock_get_dataset_data):
...
@pytest.mark.asyncio
async def test_save_tick_data(
    mock_order_update_many,
