...
line_chart.title = 'Renewable energy sources output in kW'
...
line_chart.add('GEOTHERMAL', list(
    zip(map(lambda d: date(d.year, d.month, d.day), df.index.to_series().tolist()
            ), df['GEOTHERMAL'].tolist())))
line_chart.add('WIND', list(
    zip(map(lambda d: date(d.year, d.month, d.day), df.index.to_series().tolist()
            ), df['WIND'].tolist())))
line_chart.add('SOLAR', list(
    zip(map(lambda d: date(d.year, d.month, d.day), df.index.to_series().tolist()
            ), df['SOLAR'].tolist())))
line_chart.add('HYDRO', list(
    zip(map(lambda d: date(d.year, d.month, d.day), df.index.to_series().tolist()
            ), df['HYDRO'].tolist()))
