...
def test_price_tracker(get_order, get_timestamp):
    ...
    orderbook.add_order(get_order(trader_id=5, price=30, size=50, order_side=OrderSide.BUY))
    orderbook.add_order(get_order(trader_id=6, price=30, size=50, order_side=OrderSide.SELL))
